...
class _ProfileComponentState extends State<ProfileComponent> {
  ...
  // List<Post> reviewPosts = []; // No longer needed as we use streams
  // List<PostQA> qaPosts = []; // No longer needed as we use streams
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  final user = FirebaseAuth.instance.currentUser;
  // bool _isLoading = true; // No longer needed as we use streams

  @override
  void initState() {
    super.initState();
    // _fetchUserReviews(); // No longer needed as we use streams
  }

  // No longer need these functions as we use streams
  // Future<void> _fetchUserReviews() async { ... }
  // Future<void> _fetchUserQA() async { ... }
  ...
  Future<void> _deletePost(String docId, String collection) async {
    try {
      ...
      // No need to call _fetchUserReviews or _fetchUserQA here, as the stream will handle updates
    } catch (e) {
  ...
  Stream<List<Post>> _getUserReviewsStream() {
    return _firestore
        .collection('reviews')
  ...
  Stream<List<PostQA>> _getUserQAStream() {
    return _firestore
        .collection('qandas')
        .where('userId', isEqualTo: user?.uid)
        .snapshots()
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: selectedItem == "REVIEW"
          ? StreamBuilder<List<Post>>(
              stream: _getUserReviewsStream(),
          ...
          : StreamBuilder<List<PostQA>>(
              ...
              builder: (context, snapshot) {
                ...
                if (!snapshot.hasData || snapshot.data!.isEmpty) {
                  return const Center(
                      child: Text("You haven't posted any Q&A yet."));
                }
                return PostQAList(
                    posts: snapshot.data!,
                    onBookmark: _toggleBookmark,
...
// Reuse Post, PostQA, PostCard, and PostCardQA from HomeComponent
class Post {
